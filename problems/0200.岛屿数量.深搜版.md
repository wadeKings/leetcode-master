...
```
```java
// 版本三: 在版本二的基础上去除了visited数组,改成直接修改grid数组
...
public class Main {
    ...
    static Stack<Integer> element = new Stack<>();
    // shangxiazuoyou:x,y
    ...
    public static void main(String[] args) throws InterruptedException {
        ...
        scanner.nextLine();
        int[][] grep = new int[n][m];
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < m; j++) {
                grep[i][j] = scanner.nextInt();
            }
    ...
    static void dfs(int[][] grep , int[][] visited , int x ,int  y , int m , int n){
        // shangxiazuoyou
        //4 0
        for (int i = 0; i < 4; i++) {
            int px = x + move[2*i];
            int py = y + move[2*i+1];
            //if(0 > x || m < x || 0 > y || n <y) continue;
            if (0 <= px && m > px && 0 <= py && n > py && grep[py][px] == 1) {
                grep[py][px] = 0 ;
                dfs(grep, visited,px,py ,m,n);
            }
        }
    }
